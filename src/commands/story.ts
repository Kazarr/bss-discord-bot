...
export const storyCommand = new SlashCommandBuilder()
  .setName('story')
  .setDescription('INVEST-style user story generation');
...
export async function handleStoryCommand(
  ...
  try {
    ...
    await threadService.sendMessage(
      thread,
      'Aký user story by si chcel/a vytvoriť?'
    );
    ...
    console.error('Error creating story thread:', errorMessage);
